...
contract GenesisProtocol is IntVoteInterface {
    ...
    event Stake(bytes32 indexed _proposalId, address indexed _organization, address indexed _voter,uint _vote,uint _amount);
   ...
   */
        ...
        returns(bytes32)
    {
        ...
        proposal.organization = parameters[_paramsHash].organization;
   ...
   */
    ...
    function stake(bytes32 _proposalId, uint _vote, uint _amount) external returns(bool) {
      ...
      // Event:
        emit Stake(_proposalId, proposal.organization, msg.sender, _vote, _amount);
   ...
   */
    function execute(bytes32 _proposalId) public votable(_proposalId) returns(bool) {
        ...
        uint totalReputation = GenesisProtocolCallbacksInterface(proposal.organization).getTotalReputationSupply();
    ...
    function redeem(bytes32 _proposalId,address _beneficiary) public returns(bool) {
        ...
        if (reputation != 0 ) {
            GenesisProtocolCallbacksInterface(proposal.organization).mintReputation(reputation,_beneficiary);
    ...
    function redeemDaoBounty(bytes32 _proposalId,address _beneficiary) public returns(bool) {
        ...
        if (amount != 0) {
            ...
            require(GenesisProtocolCallbacksInterface(proposal.organization).stakingTokenTransfer(_beneficiary,amount));
            emit RedeemDaoBounty(_proposalId,proposal.organization,_beneficiary,amount);
        }
        return true;
    }
    ...
    function getRedeemableTokensStakerBounty(bytes32 _proposalId,address _beneficiary) public view returns(uint) {
        ...
        uint bounty = (params.daoBountyParams[0].mul(proposal.stakers[_beneficiary].amountForBounty))/100;
   ...
   */
    ...
    function internalVote(bytes32 _proposalId, address _voter, uint _vote, uint _rep) private returns(bool) {
        ...
        uint reputation = GenesisProtocolCallbacksInterface(proposal.organization).reputationOf(_voter);
        ...
        if (proposal.state != ProposalState.Boosted) {
            ...
            GenesisProtocolCallbacksInterface(proposal.organization).burnReputation(reputationDeposit,_voter);
        }
        // Event:
